...
(function(){
const STORAGE='royalTigerDashboardDraftV1';
...
function save(){localStorage.setItem(STORAGE,JSON.stringify({july:weeks.july,w30:weeks[30],w29:weeks[29],julyPeople:JULY_DATA.people,week30People:people}))}
function load(){try{const d=JSON.parse(localStorage.getItem(STORAGE));if(!d)return;if(d.july)Object.assign(weeks.july,d.july);if(d.w30)Object.assign(weeks[30],d.w30);if(d.w29)Object.assign(weeks[29],d.w29);if(d.julyPeople)JULY_DATA.people.splice(0,JULY_DATA.people.length,...d.julyPeople);if(d.week30People)people.splice(0,people.length,...d.week30People)}catch(e){console.warn('draft ignored',e)}}
...
function openEditor(){
 ...
 html+='<div class="edit-block"><h3>核心数据</h3><div class="edit-grid">'+field(m,'sales','销售额')+field(m,'fees','活动费用')+field(m,'events','活动场次','1')+field(m,'personDays','临促人员天数','1')+field(m,'humanHitRate','人效达标率')+field(m,'visits','巡店登记','1')+(w.isMonth?field(m,'uniqueStores','覆盖门店','1'):'')+'</div></div>';
 html+=table('活动类型',w.levels,[['events','场次','1'],['sales','销售额'],['fees','费用'],['personDays','人天','1'],['hitRate','达标率']]);
 html+=table('城市经营',w.cities,[['events','场次','1'],['sales','销售额'],['fees','费用'],['personDays','人天','1'],['hitRate','达标率'],['visits','巡店数','1']]);
 if(w.isMonth)html+=table('周度趋势',w.weekly,[['events','场次','1'],['sales','销售额'],['fees','费用'],['personDays','人天','1'],['hitRate','达标率'],['visits','巡店数','1']]);
 if(w!==weeks[29])html+=table('巡店人员',w.isMonth?JULY_DATA.people:people,[['visits','巡店数','1'],['unique','覆盖门店','1'],['target','目标','1'],['score','评分']]);
